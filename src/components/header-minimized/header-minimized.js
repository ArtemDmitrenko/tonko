export default class HeaderMinimized {
  ...
  init() {
    ...
    this.productionButton = this.item.querySelector(
      ".js-header__minimized-header .js-nav-menu__link"
    );
    this.allSubMenuButtons = this.item.querySelectorAll(".js-sub-header__link");
    this.subHeader = this.item.querySelector(
      ".js-header-minimized__sub-header"
    );
    this.headerMinimized = this.item.querySelector(
      ".js-header__minimized-header"
    );
    this.minimizedHeaderIsOn = false;
  }

  addEventListeners() {
    this.productionButton.addEventListener(
      "mouseover",
      this.handleProductionButtonHover
    );
    document.addEventListener(
      "scroll",
      this.handlePageScrollForMinimizedHeader
    );
  }
  ...
  handlePageScrollForMinimizedHeader = () => {
    ...
    if (window.pageYOffset > offset && !this.minimizedHeaderIsOn) {
      this.headerMinimized.style.maxHeight =
        this.headerMinimized.scrollHeight + "px";
